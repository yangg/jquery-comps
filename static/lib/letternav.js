...
(function() {
    function LetterNav(panel, config) {
        this.panel = $(panel);
        this.config = $.extend({
            anchorSelector: 'dt'
        }, config);
    ...
    LetterNav.prototype = {
        init: function() {
            var anchorHtml = new strbuf();
            this.panel.find(this.config.anchorSelector).each(function(){
                anchorHtml.push('<div>{0}</div>', $(this).text());
            });
            var anchorList = $('.anchor-list').html(anchorHtml.toString());
            setTimeout(function() {
                anchorList.css({marginTop: -anchorList.height()/2}); // vertical middle
            }, 0);
        ...
        scrollTo: function(target) {
            ...
            var anchorTarget = $('#anchor_' + anchor);

            var top = anchorTarget.offset().top + this.panel.scrollTop();
            // disable -webkit-overflow-scrolling: touch
            // before set scrollTop to avoid performance issue
            this.panel.addClass('scrolling')
                .scrollTop(top)
                .removeClass('scrolling');
        },
        bindEvent: function() {
            this.anchorList
                .on('touchstart MSPointerDown pointerdown', $.proxy(function(e) {
                ...
                .on('touchmove MSPointerMove pointermove', $.proxy(function(e) {
                ...
                }, this)).on('touchend MSPointerUp pointerup', $.proxy(function() {
                    this.touchData = null;
                }, this));
        }
    };
